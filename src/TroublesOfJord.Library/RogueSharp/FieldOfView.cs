...
namespace RogueSharp
{
	...
	public class FieldOfView<TCell> where TCell : ICell
	{
		...
		public HashSet<int> CellIndices { get; private set; }
		...
		{
			return CellIndices.Contains(_map.IndexFor(x, y));
		}
		...
		public ReadOnlyCollection<TCell> AppendFov(int xOrigin, int yOrigin, int radius, bool lightWalls)
		{
			foreach (TCell borderCell in _map.GetBorderCellsInSquare(xOrigin, yOrigin, radius))
			{
				...
				{
					int deltaX = cell.X - xOrigin;
					int deltaY = cell.Y - yOrigin;
					var dist = (int)Math.Sqrt(deltaX * deltaX + deltaY * deltaY);
					if (dist > radius)
					{
						break;
					}
					...
					{
						CellIndices.Add(_map.IndexFor(cell));
					}
					else
					{
						if (lightWalls)
		...
		private void PostProcessFovQuadrant(int x, int y, Quadrant quadrant)
		{
			...
			if (!IsInFov(x, y) && !_map.IsTransparent(x, y))
			{
				...
				{
					CellIndices.Add(_map.IndexFor(x, y));
				}
			}
		}
