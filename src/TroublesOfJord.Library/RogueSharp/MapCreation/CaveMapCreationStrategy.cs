...
namespace RogueSharp.MapCreation
{
	...
	public class CaveMapCreationStrategy<TMap, TCell> : IMapCreationStrategy<TMap, TCell> where TMap : class, IMap<TCell>, new() where TCell : ICell
	{
		...
		public CaveMapCreationStrategy(ICellInfo space, ICellInfo wall, int width, int height, int fillProbability, int totalIterations, int cutoffOfBigAreaFill, IRandom random)
		{
			if (space == null)
			{
		...
		private void RandomlyFillCells()
		{
			foreach (TCell cell in _map.GetAllCells())
			{
				...
				{
					_map.SetCellInfo(cell.X, cell.Y, _wall);
				}
				else if (_random.Next(1, 100) < _fillProbability)
				{
					_map.SetCellInfo(cell.X, cell.Y, _space);
				}
				else
				{
					_map.SetCellInfo(cell.X, cell.Y, _wall);
		...
		private static TMap Clone<TMap>(TMap map) where TMap : IMap<TCell>, new()
		{
			...
			{
				result.SetCellInfo(cell.X, cell.Y, cell.Info);
			}
			return result;
		}
		...
		private void CellularAutomataBigAreaAlgorithm()
		{
			...
			foreach (TCell cell in _map.GetAllCells())
			{
				...
				{
					updatedMap.SetCellInfo(cell.X, cell.Y, _wall);
				}
				else
				{
					updatedMap.SetCellInfo(cell.X, cell.Y, _space);
				}
			}
				...
				for (int i = 0; i < mapSections.Count; i++)
				{
					...
					foreach (TCell cell in tunnelCells)
					{
						...
						if (previousCell != null)
						{
							...
							{
								_map.SetCellInfo(cell.X + 1, cell.Y, _space);
							}
						}
						previousCell = cell;
					}
		...
		private class FloodFillAnalyzer
		{
			...
			private MapSection Visit(TCell cell)
			{
				...
				{
					cell = stack.Pop();
					if (_visited[cell.Y][cell.X] || !cell.Info.IsWalkable)
					{
						continue;
					}
					...
					{
						if (cell.Info.IsWalkable == neighbor.Info.IsWalkable && !_visited[neighbor.Y][neighbor.X])
						{
							stack.Push(neighbor);
						}
					}
