...
namespace RogueSharp.MapCreation
{
	...
	public class RandomRoomsMapCreationStrategy<TMap, TCell> : IMapCreationStrategy<TMap, TCell> where TMap : IMap<TCell>, new() where TCell : ICell
	{
		...
		public RandomRoomsMapCreationStrategy(ICellInfo space, ICellInfo wall, int width, int height, int maxRooms, int roomMaxSize, int roomMinSize, IRandom random)
		{
			if (space == null)
			{
		...
		public RandomRoomsMapCreationStrategy(ICellInfo space, ICellInfo wall, int width, int height, int maxRooms, int roomMaxSize, int roomMinSize): 
			this(space, wall, width, height, maxRooms, roomMaxSize, roomMaxSize, Singleton.DefaultRandom)
		{
		}
		...
		private void MakeRoom(TMap map, Rectangle room)
		{
			for (int x = room.Left + 1; x < room.Right; x++)
			{
				for (int y = room.Top + 1; y < room.Bottom; y++)
				{
					map.SetCellInfo(x, y, _space);
				}
			}
		}

		private void MakeHorizontalTunnel(TMap map, int xStart, int xEnd, int yPosition)
		{
			for (int x = Math.Min(xStart, xEnd); x <= Math.Max(xStart, xEnd); x++)
			{
				map.SetCellInfo(x, yPosition, _space);
			}
		}

		private void MakeVerticalTunnel(TMap map, int yStart, int yEnd, int xPosition)
		{
			for (int y = Math.Min(yStart, yEnd); y <= Math.Max(yStart, yEnd); y++)
			{
				map.SetCellInfo(xPosition, y, _space);
			}
		}
	}
}