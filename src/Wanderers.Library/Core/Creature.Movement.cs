	...
	partial class Creature
	{
		...
		public void Enter()
		{
			...
			if (TJ.Module.MapTemplates.ContainsKey(Tile.Exit.MapId))
			{
				...
				var generator = TJ.Module.EnsureGenerator(mapTemplate.GeneratorId);
				...
				{
					throw new Exception(string.Format("Could not find free tile at map generated by generator '{0}'", mapTemplate.GeneratorId));
				}

				exitTile = freeTiles[MathUtils.Random.Next(0, freeTiles.Count)];

				exitTile.Info = TJ.Module.EnsureTileInfo("ExitUp");
				exitTile.Exit = new Exit
				{
					MapId = Map.Id,
					Position = Position
				};
